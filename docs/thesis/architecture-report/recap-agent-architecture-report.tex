...
\newcommand{\ps@recaplogos}{%
	\renewcommand{\@oddhead}{\brandlogo{utec-logo.pdf}{UTEC}\hfill\small Informe técnico\hfill\brandlogo{sin-envolturas-logo.pdf}{Sin Envolturas}}%
...
\begin{fullwidth}
	...
	\brandlogo{utec-logo.pdf}{UTEC}\hfill\brandlogo{sin-envolturas-logo.pdf}{Sin Envolturas}
...
\begin{tikzpicture}[x=1cm,y=1cm, every node/.style={font=\footnotesize}]
	\tikzset{
		channel/.style={draw=DimGray, rounded corners=2pt, thick, fill=WhiteSmoke, align=center, minimum width=2.65cm, minimum height=0.85cm},
		api/.style={draw=DarkBlue, rounded corners=2pt, thick, fill=AliceBlue, align=center, minimum width=3.0cm, minimum height=0.85cm},
		aws/.style={draw=DarkOrange, rounded corners=2pt, thick, fill=OldLace, align=center, minimum width=3.0cm, minimum height=0.85cm},
		data/.style={draw=DarkGreen, rounded corners=2pt, thick, fill=Honeydew, align=center, minimum width=3.0cm, minimum height=0.85cm},
		openai/.style={draw=DarkRed, rounded corners=2pt, thick, fill=MistyRose, align=center, minimum width=3.0cm, minimum height=0.85cm},
		groupbox/.style={draw=gray, rounded corners=4pt, dashed, thick},
		arrow/.style={-{Latex[length=2.2mm]}, thick, draw=DimGray}
	}

	\draw[groupbox] (4.7,-2.85) rectangle (12.8,5.55);
	\node[font=\small\bfseries] at (8.75,5.85) {AWS - entorno serverless};
	\draw[groupbox] (13.45,2.55) rectangle (17.45,5.25);
	\node[font=\small\bfseries] at (15.45,5.55) {OpenAI};
	\draw[groupbox] (13.45,0.15) rectangle (17.45,1.8);
	\node[font=\small\bfseries] at (15.45,2.1) {Servicios Sin Envolturas};

	\node[channel] (wa) at (0,4.65) {WhatsApp\\Webhook};
	\node[channel] (web) at (0,3.25) {Web client};
	\node[channel] (cli) at (0,1.85) {CLI y evals};
	\node[api] (adapter) at (3.1,3.25) {Adaptador\\agnóstico de canal};

	\node[aws] (url) at (6.35,3.25) {AWS Lambda\\Function URL};
	\node[aws] (lambda) at (10.0,3.25) {AWS Lambda\\recap-agent runtime};
	\node[data] (plans) at (6.35,4.65) {Amazon DynamoDB\\planes};
	\node[data] (perf) at (10.0,4.65) {Amazon DynamoDB\\telemetría};
	\node[aws] (secrets) at (6.35,1.75) {AWS Secrets\\Manager};
	\node[aws] (logs) at (10.0,1.75) {Amazon\\CloudWatch};

	\node[aws] (events) at (6.35,-0.65) {Amazon\\EventBridge};
	\node[aws] (syncp) at (9.95,-0.65) {Lambda\\provider sync};
	\node[aws] (synck) at (9.95,-2.0) {Lambda\\knowledge sync};
	\node[data] (s3) at (6.35,-2.0) {Amazon S3\\artefactos};

	\node[openai] (agent) at (15.45,4.55) {OpenAI Agents\\respuesta + extractor};
	\node[openai] (vectors) at (15.45,3.25) {OpenAI Vector Stores\\proveedores + FAQ};
	\node[api] (seapi) at (15.45,0.95) {APIs Sin Envolturas\\proveedores e invitados};

	\draw[arrow] (wa.east) -- (adapter.west);
	\draw[arrow] (web.east) -- (adapter.west);
	\draw[arrow] (cli.east) -- (adapter.west);
	\draw[arrow] (adapter.east) -- (url.west);
	\draw[arrow] (url.east) -- (lambda.west);
	\draw[arrow] (lambda.north) -- (perf.south);
	\draw[arrow] (lambda.north west) -- (plans.south east);
	\draw[arrow] (lambda.south west) -- (secrets.north east);
	\draw[arrow] (lambda.south) -- (logs.north);
	\draw[arrow] (lambda.east) -- (agent.west);
	\draw[arrow] (lambda.east) -- (vectors.west);
	\draw[arrow] (lambda.east) -- ++(1.2,0) |- (seapi.west);
	\draw[arrow] (events.east) -- (syncp.west);
	\draw[arrow] (events.east) -- ++(0.8,0) |- (synck.west);
	\draw[arrow] (syncp.east) -- ++(1.0,0) |- (vectors.west);
	\draw[arrow] (syncp.east) -- ++(1.0,0) |- (seapi.west);
	\draw[arrow] (synck.east) -- ++(1.25,0) |- (vectors.west);
	\draw[arrow] (synck.west) -- (s3.east);
\end{tikzpicture}
}
\caption{Vista de despliegue AWS y relación con canales, OpenAI y APIs de Sin Envolturas. Colores: gris para canales, naranja para cómputo AWS, verde para datos, rojo para OpenAI y azul para APIs externas.}
